...
@BackboneRegistry.register("ncsnpp_drift")
class NCSNppdrift(nn.Module):
    """NCSN++ model, adapted from https://github.com/yang-song/score_sde repository"""

    @staticmethod
    def add_argparse_args(parser):
        #parser.add_argument("--ch_mult1",type=int, nargs='+', default=[1,1,2,2,2,2,2])
        #parser.add_argument("--num_res_blocks1", type=int, default=2)
        #parser.add_argument("--attn_resolutions1", type=int, nargs='+', default=[16])
        #parser.add_argument("--no-centered1", dest="centered", action="store_false", help="The data is not centered [-1, 1]")
        #parser.add_argument("--centered1", dest="centered", action="store_true", help="The data is centered [-1, 1]")
        parser.set_defaults(centered1=True)
        return parser
        ...
        **unused_kwargs
        ...
        if resblock_type == 'ddpm':
            ...
            ResnetBlock = functools.partial(ResnetBlockBigGAN, act=act,
                ...
                init_scale=init_scale, skip_rescale=skip_rescale, temb_dim=nf * 4,step_size_dim=nf *4)
    ...
    def forward(self, x, time_cond=None,step_size=None):
        ...
        if self.embedding_type == 'fourier':
            ...
            step_size  = modules[m_idx](torch.log(step_size))
            m_idx += 1
        ...
        if self.conditional:
            ...
            step_size = modules[m_idx](step_size)
            m_idx += 1
            temb = modules[m_idx](self.act(temb))
            step_size = modules[m_idx](self.act(step_size))
            m_idx += 1
        else:
            temb = None
            step_size=None
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                h = modules[m_idx](hs[-1], temb,step_size)
                m_idx += 1
        ...
        h = modules[m_idx](h, temb,step_size)  # ResNet block
        m_idx += 1
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = modules[m_idx](torch.cat([h, hs.pop()], dim=1), temb,step_size)
                m_idx += 1
            ...
            if i_level != 0:
                if self.resblock_type == 'ddpm':
                    ...
                    h = modules[m_idx](h, temb,step_size)  # Upspampling
                    m_idx += 1
